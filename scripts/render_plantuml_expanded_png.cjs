...
async function main() {
  ...
  const pumlPath = path.join(repoRoot, 'diagrams', 'architecture_expanded.puml');
  const outPng = path.join(repoRoot, 'diagrams', 'architecture_expanded.png');
  ...
  try {
    ...
    console.error('Failed to fetch PNG:', err.message);
    process.exit(2);
  }
}
